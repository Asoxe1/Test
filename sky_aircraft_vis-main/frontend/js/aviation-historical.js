...
// Use same-origin so the main server can proxy to the scrapping service
...
async function fetchFiles(){
  ...
  try{
    const res = await fetch(`${SOCKET_URL}${API_BASE}/files`);
    if (!res.ok) throw new Error('API files error');
    const files = await res.json();
    sel.innerHTML = '';
    files.forEach(f => { const opt = document.createElement('option'); opt.value = f; opt.textContent = f.replace('positions_','').replace('.csv','').replace('_',' '); sel.appendChild(opt); });
  }catch(e){ sel.innerHTML = '<option>Erreur</option>'; console.error(e); }
}

function parseCsvData(csvText){
  // Many historical CSVs are produced without a header row. Use csvParseRows
  // and map columns by index to expected fields.
  const rows = d3.csvParseRows(csvText);
  const mapped = rows.map(r => {
    // defensive: some rows may be shorter
    const ts_iso = r[0] || '';
    const icao24 = r[1] || '';
    const callsign = r[2] || '';
  ...
  return mapped.filter(d => isFinite(Number(d.lat)) && isFinite(Number(d.lon)));
}

async function loadAndRender(filename){
  showLoader();
  try{
    const res = await fetch(`${SOCKET_URL}${API_BASE}/data/${filename}`);
    if (!res.ok) throw new Error('Failed to fetch ' + filename);
    const text = await res.text();
    const arr = parseCsvData(text);
    // set lastDisplayedBatch and render using shared renderer
    lastDisplayedBatch = arr;
    processBatchAndRender(lastDisplayedBatch);
    document.getElementById('lastUpdate').textContent = 'Chargé: '+filename;
